(function() {
  var _, async, collectModules, defaultLoader, exportModule, firstChunk, mergeOptionsFile, path, rjs, through, trace, vinylFs;
      ...
      return async.waterfall([
        ...
        excludedModules) {
              ...
              if (err) {
                ...
                excludedNames = _(excludedModules).map(function(module) {
                  return collectModules(module);
                }).flatten().map("name").uniq().value();
                return callback(null,
        modules,
        excludedNames);
              }
