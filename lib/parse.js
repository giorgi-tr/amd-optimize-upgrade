(function() {
  var _, acorn, ancestor, escodegen, parseRequireDefinitions, simple, valuesFromArrayExpression;
  ...
  ancestor = require("acorn-walk").ancestor;

  simple = require("acorn-walk").simple;
    ...
    ancestor(ast, {
      CallExpression: function(node, state) {
        ...
        if ((node.callee.name === "require" || node.callee.name === "requirejs") && node.arguments.length > 0 && node.arguments[0].type === "ArrayExpression") {
          defineAncestors = _.some(state.slice(0, -1), function(ancestorNode) {
